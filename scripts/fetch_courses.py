...
def parse_enext_list(html_text: str, base: str) -> list[dict]:
    ...
    for card in cards:
        ...
        if m:
            candidate = clean_text(m.group(1))[:60]
            if candidate.lower() not in {"category", "subcategory", "sub-category", "n/a", ""}:
                category = candidate
        if category.lower() in {"category", "subcategory", "it & software"}:
            for cat in (
                "Development",
                "Business",
                "Design",
    ...
    try:
        ...
        for line in text.splitlines():
            ...
            if low.startswith("language"):
                ...
                val = line.split(None, 1)[-1][:80].strip()
                if val.lower() not in {"category", "subcategory", "n/a", ""}:
                    meta["category"] = val
...
<head>
  ...
  <title>{title} — Free Udemy Coupon | SearchKaro</title>
...
def main() -> None:
    ...
    for i, item in enumerate(collected[:max_detail]):
        ...
        category = item.get("category") or "IT & Software"
        meta_cat = (meta.get("category") or "").strip()
        if meta_cat and meta_cat.lower() not in {"category", "subcategory", "n/a"}:
            category = meta_cat
        if category.lower() in {"category", "subcategory"}:
            category = "IT & Software"
